...
var Alert = function(opts, extra) {
  ...
  switch(opts.mode) {
    ...
    case 'smart':
      ...
      this.cmp = function(value, threshold) {
        if (value !== null) {
          this.tdigest.push(parseFloat(value));
          debug('Check', value, this.tdigest.p_rank(value), Math.abs(this.tdigest.p_rank(value) - 0.5));

          if (this.start)
          // Start if > percentile 95 or < 5
            return (Math.abs(this.tdigest.p_rank(value) - 0.5) > 0.475);
        }
        return false;
      };
      break;
