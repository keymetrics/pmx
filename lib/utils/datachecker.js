...
function dataChecker(opts) {
  ...
  this._computeDev = func[this._calcDev];
...
dataChecker.prototype.normalDev = function() {
  return ((self._refresh() - self._histogram.getEma()) / self._histogram.getEma() > self._dev);
}

//Calculate deviation of current EMA compared to Mean
dataChecker.prototype.medianDev = function() {
  return ((self._histogram.getEma() - self._histogram._calculateMean()) / self._histogram._calculateMean() > self._dev);
}
...
dataChecker.prototype.start = function() {
  ...
  this._interval_timer = setInterval(function() {
    ...
    if (self._monitored == true && (self._refresh() - self._histogram.getEma()) / self._histogram.getEma() > self._dev) {
      self._counter++;
      /**
