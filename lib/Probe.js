...
Probe.AVAILABLE_AGG_TYPES  = ['avg', 'min', 'max', 'sum'];
Probe.AVAILABLE_MESUREMENTS = [
  'min',
  'max',
  'sum',
  'count',
...
function cookData(data) {
  ...
  Object.keys(data).forEach(function(probe_name) {
    ...
    cooked_data[probe_name] = {
      value: value,
      agg_type: data[probe_name].agg_type
    }
  });
  return cooked_data;
};
...
Probe.probe = function() {
  ...
  return {
    ...
    histogram : function(opts) {
      ...
      if (Probe.AVAILABLE_MESUREMENTS.indexOf(opts.measurement) == -1)
