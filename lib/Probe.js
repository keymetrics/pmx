  ...
  Object.keys(data).forEach(function(probe_name) {
    ...
    if (data[probe_name].alert) {
      cooked_data[probe_name].alert = {
        threshold : data[probe_name].alert.threshold
      };

      if (data[probe_name].alert.mode === 'threshold-avg')
        cooked_data[probe_name].alert.interval = data[probe_name].alert.sample._size;
    }
  });
  return cooked_data;
};
